...
describe('TableBuilder Private Methods', function () {
    ...
    describe('generateTableHash()', function () {
        it('generates hash based on table dimensions', function () {
            $builder = new TableBuilder();
            ...
            $hash = $method->invoke($builder, $table);
            
            expect($hash)->toBeString();
            expect(strlen($hash))->toBe(32);
        });

        it('generates same hash for tables with same dimensions', function () {
            $builder = new TableBuilder();
        ...
        it('generates different hash for different dimensions', function () {
            $builder = new TableBuilder();
        ...
        it('throws exception for table with no rows', function () {
            $builder = new TableBuilder();
            
            // Create a mock table with no rows
            $section = (new ContentControl())->addSection();
            $emptyTable = $section->addTable();
            
            $reflection = new ReflectionClass($builder);
            $method = $reflection->getMethod('generateTableHash');
            $method->setAccessible(true);
            
            expect(fn() => $method->invoke($builder, $emptyTable))
                ->toThrow(ContentControlException::class, 'Cannot generate hash for empty table');
        });
    });
